  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const memberIds = new Set(courseMembers.map((member) => member.iduser));
        const filtered = response.data.filter((person) =>
          person.iduser !== user?.id && !memberIds.has(person.iduser)
        );
    ...
    fetchUsers();
  }, [showAddStudent, user?.access_token, user?.id, courseMembers]);
